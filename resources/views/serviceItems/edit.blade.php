...
<!--- Form for editing an existing service item with fields for title, category, price, description, and notes --->
<form method="POST" action="/serviceItems/edit/{{ $model->Id }}">
    @csrf
    ...
    <select name="ServiceCategoryId">
        ...
        @foreach($categories as $category)
            <option value="{{ $category->Id }}"
                {{ old('ServiceCategoryId', $model->ServiceCategoryId) == $category->Id ? 'selected' : '' }}>
                {{ $category->Title }}
            </option>
        @endforeach
    </select>
    ...
    <label>Price</label>
    <input type="number" step="0.01" min="0" name="Price" value="{{ old('Price', $model->Price) }}">
    @error('Price')
        <div class="error">{{ $message }}</div>
    @enderror
